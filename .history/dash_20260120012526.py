...
# Import Safety Net classes for joblib to deserialize the model
import sys
sys.path.insert(0, '.')
from importlib import import_module
try:
    safety_net_module = import_module('04_safety_net')
    BulletproofSafetyNet = safety_net_module.BulletproofSafetyNet
    StatisticalBoundsDetector = safety_net_module.StatisticalBoundsDetector
    PhysicsConstraintDetector = safety_net_module.PhysicsConstraintDetector
    EnsembleIsolationDetector = safety_net_module.EnsembleIsolationDetector
except Exception as e:
    print(f"Warning: Could not import Safety Net classes: {e}")
    BulletproofSafetyNet = None
