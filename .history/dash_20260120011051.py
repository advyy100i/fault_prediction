...
def load_assets():
    ...
    # Load Safety Net (Isolation Forest) for unsupervised anomaly detection
    try:
        models['safety_net'] = joblib.load("models/safety_net.pkl")
        models['safety_net_scaler'] = joblib.load("models/safety_net_scaler.pkl")
    except Exception:
        models['safety_net'] = None
        models['safety_net_scaler'] = None
...
with st.sidebar:
    ...
    st.divider()
    st.header("🛡️ Safety Net")
    st.markdown("""
    **Unsupervised Anomaly Detection**
    
    The Safety Net uses Isolation Forest to detect:
    - Unknown failure modes
    - Physics-violating states
    - Sensor anomalies
    
    *Trained only on healthy machines to learn "normality"*
    """)
...
# SAFETY NET SECTION
# =============================================================================
st.header("🛡️ Safety Net: Unsupervised Anomaly Detection")

if models.get('safety_net') is not None and models.get('safety_net_scaler') is not None:
    safety_net = models['safety_net']
    safety_scaler = models['safety_net_scaler']
    
    st.markdown("""
    The **Safety Net** is an Isolation Forest trained *only on healthy machines*.
    It learns the "shape" of normal operation and flags anything outside that boundary.
    
    **Why this matters:** The Sentry (XGBoost) can only detect *known* failure modes.
    The Safety Net catches *unknown* anomalies, physics violations, and novel failures.
    """)
    
    # Prepare data for visualization
    # Get available Safety Net features from test set
    ...
    if len(available_features) == len(SAFETY_NET_FEATURES):
        X_safety = X_test[SAFETY_NET_FEATURES].copy()
        X_safety_scaled = safety_scaler.transform(X_safety)
        
        # Get anomaly predictions and scores
        anomaly_preds = safety_net.predict(X_safety_scaled)
        anomaly_scores = safety_net.decision_function(X_safety_scaled)
        ...
        Z = safety_net.decision_function(mesh_original).reshape(xx.shape)
        
        # Plot decision boundary
        contour = ax.contourf(xx, yy, Z, levels=30, cmap='RdYlBu', alpha=0.5)
        ...
        ax.scatter(X_pca[normal_mask, 0], X_pca[normal_mask, 1], 
                   c='blue', s=15, alpha=0.4, label=f'Normal (n={n_normal})')
        ax.scatter(X_pca[anomaly_mask, 0], X_pca[anomaly_mask, 1], 
                   c='red', s=40, alpha=0.8, label=f'Anomaly (n={n_anomalies})', marker='X')
        
        ax.set_xlabel(f'PC1 ({pca.explained_variance_ratio_[0]*100:.1f}% variance)')
        ax.set_ylabel(f'PC2 ({pca.explained_variance_ratio_[1]*100:.1f}% variance)')
        ax.set_title('Isolation Forest: Normal Region (Blue) vs Anomaly Region (Red)')
...
if uploaded:
    ...
    # Safety Net check on uploaded data
    if models.get('safety_net') is not None and models.get('safety_net_scaler') is not None:
        st.subheader("🛡️ Safety Net Analysis")
        ...
        if len(safety_features_available) == len(SAFETY_NET_FEATURES):
            safety_data = data_in[SAFETY_NET_FEATURES]
            safety_scaled = models['safety_net_scaler'].transform(safety_data)
            safety_preds = models['safety_net'].predict(safety_scaled)
            safety_scores = models['safety_net'].decision_function(safety_scaled)
            
            n_anomalies = (safety_preds == -1).sum()
            st.write(f"**Safety Net detected {n_anomalies} anomalies** out of {len(data_in)} samples")
